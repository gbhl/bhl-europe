...
<tr><td colspan=".$tabCols." align=center>";

?>

<div style="margin-top: 5px; margin-bottom: 5px;" class="demo">
...
lz(2);  
echo "<a href=\"#\" onClick=\"switch_visibility(document.getElementById('pd_div'));\"><font style=\"font: 9px arial; color: #aaaaaa;\">";
...
$query = "select content_id, date_format(content_ctime,'%Y.%m.%d %H:%i'), 
    date_format(content_atime,'%Y.%m.%d %H:%i'), 
...
if ($nrows>0)
{
    ...
    while ($line)
    {
        ...
        // DATES
        echo "<br>".$line[1]."<br><font color=\"#aaaaaa\">".$line[2]."</font>"._TD;
        ...
        lz();
        icon("folder_16.png","Explore its Directory. ".str_replace(_CONTENT_ROOT,"",$line[3]),"onClick=\"javascript: popup_win('ft','"._SYSTEM."?menu_nav=file_tree&start_root=".urlencode(clean_path(str_replace(_CONTENT_ROOT,"",$line[3]))."/")."',560,440);\"");
       
        nl(2);
        echo _TD;
        
        // CONTENT TYPE 
        arr_dropdown($arrEnumCTypes,"content_type' disabled style='width:120px;",$line[4],1,"","",true);
        nl();
        
        // INGEST STATUS
        arr_dropdown($arrEnumCStatus,"content_status_".$line[0]."' disabled style='width:120px;",$line[7],1,"","",true);
        ...
        nl();
        textfeld("content_ipr_".$line[0]." style=\"margin-top: 4px; border: 1px solid green; text-align: center; width: 150px; font-weight: normal;\" ",$line[11],19,254,"","",false);
