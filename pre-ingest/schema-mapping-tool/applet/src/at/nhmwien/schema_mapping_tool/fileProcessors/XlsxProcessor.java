...
public class XlsxProcessor extends FileProcessor {
    private XSSFWorkbook readBook = null;
    private XSSFSheet readSheet = null;
    ...
    public void nextEntry() throws Exception {
        if( this.readSheet != null ) {
            ...
            for( int i = 0; i < fieldOrder.size(); i++ ) {
                ...
                nRecord.setRecordContent( this.readSheet.getRow( this.currRow ).getCell( this.fieldNames.get( fieldOrder.get(i) ).intValue() ).toString() );
    ...
    public boolean hasNext() {
        if( this.recordsStack.size() > 0 || this.currRow < this.readSheet.getLastRowNum() ) return true;
